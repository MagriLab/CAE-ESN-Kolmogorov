...
Nx       = 48
Nu       = 2
fln      = '/rds/user/ar994/hpc-work/Kolmogorov_0.1_48_34.0_100100_32.h5'
#fln      = '/data/ar994/Python/data/Kolmogorov/Kolmogorov_0.1_48_34.0_100100_32.h5'
hf       = h5py.File(fln,'r')
dt       = 0.1
...
#fln      = 'data/U_mean.h5'
#hf = h5py.File(fln,'w')
#hf.create_dataset('U_mean'      ,data=np.mean(U, axis=0))
#hf.close()
...
for indx in idxs:
    ...
    n_fil         = [6,12,24,6]                  #size of filters in encoder
    ...
    if load:
        ...
        N_latent = 9
        path = './data/48_RE34_'+der_string+dbl_string+str(ker_size)+'_'+act+'_'+str(N_latent)+indx
        ...
        for j in range(N_parallel):
            for i in range(N_layers):
                ...
                #initial additional conv layer with stride=1 (to be similar to last layer decoder)
    #             if i==0: 
    #                 enc_mods[j].add(PerPad2D(padding=p_fin[j], asym=False,
    #                                                       name='Enc_'+str(j)+'_Add_PerPad_'+str(i)))
    #                 enc_mods[j].add(tf.keras.layers.Conv2D(filters=n_dec[-1],
    #                                                     kernel_size=ker_size[j], 
    #                                                 activation=act,padding=pad_dec,strides=1,
    #                                                     name='Enc_'+str(j)+'_Add_Layer_'+str(i)))        
