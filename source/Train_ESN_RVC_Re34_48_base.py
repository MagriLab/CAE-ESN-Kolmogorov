...
import os
os.environ["OMP_NUM_THREADS"] = '8' # imposes only one core
import numpy as np
...
N_latent   = 18
idxs       = [0]                               #for running different latent spaces of each size
N_layer    = 3
N_dim      = N_latent
act        = 'tanh'
...
for idx in idxs:
    ...
    fln  = '/rds/user/ar994/hpc-work/48_Encoded_data_Re34_base_double_' + str(ker_size) \
            + '_'+str(N_latent) + '_' + str(N_layer) + '_' +  act + '_' + str(idx) + '.h5'
    hf  = h5py.File(fln,'r')
    ...
    for N_units in N_unitss:
        ...
        for NN_train in N_trains:

            t_lyap    = .068**(-1)        # Lyapunov time
            N_lyap    = int(t_lyap/dt)  # number of time steps in one Lyapunov time
            print(N_lyap, dt, t_lyap)

            # number of time steps for washout, train, validation, test
            N_washout = 50
            N_train   = int(NN_train/dt)
            N_val     = int(100/dt)
            ...
            sigma_ph = np.sqrt(np.mean(np.std(U_tv)**2))


            Y_tv  = np.zeros((len(noises), N_train-1, N_latent)) #U[N_washout+1:N_washout+N_train+N_val]
            ...
            spec_in     = .1  #np.log10(.1)   #range for hyperparameters (spectral radius and input scaling)
            spec_end    = 1.  #np.log10(1.)  
            in_scal_in  = round(np.log10(0.1),3)
            in_scal_end = round(np.log10(5.),3)
            ...
            if restart:
                fln     = './data/48_ESN_Re34_enc_base_double_' + str(N_units) + '_' +str(N_latent)+'_'+str(NN_train)+ '_' + str(idx) + '.mat'
                
                data    = loadmat(fln)
                Woutt   = np.array(data['Wout']) 
                minimum = np.array(data['hyperparameters'])
                sigma_n = minimum[0,3]
                j_st    = data['j_st'][0][0]
                
                Winn    = []
                Ws      = []
                for k in range(len(data['Win'][0])):
                    Winn += [data['Win'][0][k]]
                    Ws   += [data['W'][0][k]]
            ...
            for j in range(j_st,ensemble):
                ...
                if j < 1:
                    ...
                    sigma_n  = 0.001*np.sqrt(10) #first value of noise
                    ...
                    #     minn       += [res.fun]
                        ...
                        #compute performance in the test set
                        rho, sigma_in = res.x
                        ...
                        for i in range(N_test):
                            ...
                            Errors[i]    = - np.argmax(Y_err>threshold_ph)    #np.log10(np.mean((Yh_t-Y_t)**2))
                            print(Errors[i])
                            
                            if Errors[i] == 0 and Y_err[0]<threshold_ph: Errors[i] = - N_intt
                    ...
                    # sigma_n  = 0.01 #first value of noise

                    # max_iterations = 4
                    U_tv     = np.empty((N_train-1, N_latent))
                    ...
                    rnd1  = np.random.RandomState(seed)
                    for i in range(N_latent):
                            U_tv[:,i] = U[N_washout:N_washout+N_train-1,i].copy() \
                                            + rnd1.normal(0, sigma_n*data_std[i], N_train-1)
                ...
                #     minn       += [res.fun]
                    ...
                    hypers       += [ [np.append(res.x,[tikk])] ]
                    sigma_in      = round(10**sigma_in,3)
                    Woutt[j]      = train_save_n(U_washout, U_tv, U[N_washout+1:N_washout+N_train],
                                          tikk,sigma_in,rho,0)
                        ...
                        #     # Mean Square Error
                        #     Yh_t         = closed_loop(N_intt-1, xa1, Wout, sigma_in, rho)[0]
                        #     Y_err        = np.linalg.norm((Yh_t-Y_t),axis=1)/sigma_ph
                        #     Errors[i]    = - np.argmax(Y_err[i]>threshold_ph)    #np.log10(np.mean((Yh_t-Y_t)**2))
                            
                        #     if Errors[i] == 0 and Y_err[0]<threshold_ph: Errors[i] = -(N_intt-1)
                ...
                if not load:
                    ...
                    with open(fln,'wb') as f:  # need 'wb' in Python3
                        ...
                        savemat(f, {'j_st': j+1})
                        # savemat(f, {"noise": sigma_n*data_std})
                        # savemat(f, {"gp": y_pred})
                    print(fln)
